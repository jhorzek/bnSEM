...
the Bayesian Network translation of banSEM and a rejection sampling approach.
...
First, let's use banSEM to find the distribution of $\eta$ for $x_2 \in (-2, -1)$
and $x_2 \in (1,3)$:
...
network <- banSEM::banSEM(lavaan_model = fit)
...
Comparing banSEM and rejection sampling, we find the following distributions for $\eta$:
